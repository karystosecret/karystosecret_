        ...
        <p className="text-4xl md:text-5xl text-white font-bold text-center mb-4 drop-shadow-md tracking-tight">Τοποθεσίες</p>
            ...
            <span className="flex-grow h-px bg-white ml-4 mr-20"></span>
                ...
                <div className='grid grid-cols-1 md:grid-cols-3 gap-y-14 gap-x-0 pb-4'>
                   ...
                   >
                     <div className="relative w-full" style={{ paddingBottom: "73%" }}>
                       <Image
                         className="brightness-[0.93] absolute rounded-md  inset-0 w-full h-full object-cover group-hover:brightness-[1.06] group-hover:contrast-[1.03]"
                         src={beach.image}
                         alt={beach.title}
                         layout="fill"
                       />
                       <div className="absolute inset-0 flex flex-col justify-start p-1 text-white ">
                         <p className="text-xl font-bold text-center mb-[51%] drop-shadow-lg">{beach.title}</p>
                  ...
                  >
                    <div className="relative w-full" style={{ paddingBottom: "73%" }}>
                      ...
                      <div className="absolute inset-0 flex flex-col justify-start p-0.5 text-white ">
                        <p className="text-xl font-bold text-center mb-[51%] drop-shadow-lg">{beach.title}</p>
                        <p className="text-sm text-center drop-shadow-lg">{beach.smallDescription}</p>
                        <p className="text-sm text-center drop-shadow-lg">{beach.region}</p>
                      </div>
                    </div>
